...
try {
    ...
    $response = [
        ...
        'total_records' => intval($totalRecords),           // Today's records only
        'all_time_records' => intval($allTimeRecords),      // All-time total
        'latest_record_id' => intval($latest['id']),
        
        // Current values (rounded to 2 decimal places)
        'current' => [
            'raw_biogas_flow' => round(floatval($latest['raw_biogas_flow']), 2),
            'raw_biogas_totalizer' => round(floatval($latest['raw_biogas_totalizer']), 2),
            'purified_gas_flow' => round(floatval($latest['purified_gas_flow']), 2),
            'purified_gas_totalizer' => round(floatval($latest['purified_gas_totalizer']), 2),
            'product_gas_flow' => round(floatval($latest['product_gas_flow']), 2),
            'product_gas_totalizer' => round(floatval($latest['product_gas_totalizer']), 2),
            
            'ch4_concentration' => round(floatval($latest['ch4_concentration']), 2),
            'co2_level' => round(floatval($latest['co2_level']), 2),
            'o2_concentration' => round(floatval($latest['o2_concentration']), 2),
            'h2s_content' => intval($latest['h2s_content']),
            'dew_point' => round(floatval($latest['dew_point']), 2),
            
            'd1_temp_bottom' => round(floatval($latest['d1_temp_bottom']), 2),
            'd1_temp_top' => round(floatval($latest['d1_temp_top']), 2),
            'd1_gas_pressure' => round(floatval($latest['d1_gas_pressure']), 2),
            'd1_air_pressure' => round(floatval($latest['d1_air_pressure']), 2),
            'd1_slurry_height' => round(floatval($latest['d1_slurry_height']), 2),
            'd1_gas_level' => round(floatval($latest['d1_gas_level']), 2),
            
            'd2_temp_bottom' => round(floatval($latest['d2_temp_bottom']), 2),
            'd2_temp_top' => round(floatval($latest['d2_temp_top']), 2),
            'd2_gas_pressure' => round(floatval($latest['d2_gas_pressure']), 2),
            'd2_air_pressure' => round(floatval($latest['d2_air_pressure']), 2),
            'd2_slurry_height' => round(floatval($latest['d2_slurry_height']), 2),
            'd2_gas_level' => round(floatval($latest['d2_gas_level']), 2),
            
            'buffer_tank_level' => round(floatval($latest['buffer_tank_level']), 2),
            'lagoon_tank_level' => round(floatval($latest['lagoon_tank_level']), 2),
            
            'feed_fm1_flow' => round(floatval($latest['feed_fm1_flow']), 2),
            'feed_fm1_totalizer' => round(floatval($latest['feed_fm1_totalizer']), 2),
            'feed_fm2_flow' => round(floatval($latest['feed_fm2_flow']), 2),
            'feed_fm2_totalizer' => round(floatval($latest['feed_fm2_totalizer']), 2),
            'fresh_water_flow' => round(floatval($latest['fresh_water_flow']), 2),
            'fresh_water_totalizer' => round(floatval($latest['fresh_water_totalizer']), 2),
            'recycle_water_flow' => round(floatval($latest['recycle_water_flow']), 2),
            'recycle_water_totalizer' => round(floatval($latest['recycle_water_totalizer']), 2),
            
            'psa_status' => intval($latest['psa_status']),
            'psa_efficiency' => round(floatval($latest['psa_efficiency']), 2),
            'lt_panel_power' => round(floatval($latest['lt_panel_power']), 2),
        ...
        'avg_1hr' => [
            ...
            'h2s_content' => intval(round($stats1hr['avg_h2s'])),
        ...
        'avg_12hr' => [
            ...
            'h2s_content' => intval(round($stats12hr['avg_h2s'])),
